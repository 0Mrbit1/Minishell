       ...
       if (tokens->token_type == WORD)
        {
            ...
            // that to check if the token starts with ----> $
            if (value[0] == '$')
            {
                // Expand the variable using the environment
                char *expanded_value = expand_variable(value, env);
                free(value);  // Free the old value
                value = expanded_value;  // Use the expanded value
            }

            if (!cmd->command)
            {
                 cmd->command = strdup(value);
            ...
            else
            {
                ...
                cmd->args[arg_count++] = strdup(value);
            }
        }
        /**/
        ...
        {
            cmd->next = parse_command(tokens->next,env); // handle pipes (next command)
            break;
        }
